...
def make_plot(studio_id):
    ...
    week_avg = df_4week.groupby((df_4week.index.dayofweek) * 24 +                                (df_4week.index.hour)).mean().rename_axis('HourOfWeek')
    ...
    if polling:
        ...
        plt.plot(curr, label='heute')

    else:

        ppl_now = 'N/A'

        plt.plot([], [], ' ', label='heute (unavailable)')

        # Plot previous year instead of today

        last_year_date = now - timedelta(days=365)

        idx_last_year = ((df.index > last_year_date - timedelta(hours=prevh)) &

                         (df.index < last_year_date + timedelta(hours=nexth)))

        

        prev_year_data = df[idx_last_year]['members']

        if not prev_year_data.empty:

            time_range_current = pd.date_range(start=now - timedelta(hours=prevh), end=now + timedelta(hours=nexth), periods=len(prev_year_data))

            plt.plot(time_range_current, prev_year_data.values, label='previous year')
    ...
    if len(weeks) > 0:
        ...
        min_len = min(len(w) for w in weeks)

        weeks = [w[:min_len] for w in weeks]
        ...
        plt.plot(times, np.nanmean(weeks, 0), linestyle='dashed', color='black', alpha=0.5, label='letzte Woche')
        ...
        plt.fill_between(times, weeksmin, weeksmax, color='gray', alpha=0.3, label=f'letzte {last_n-1} Wochen')
    ...
    if polling:
        ...
        title = f'Jetzt: {int(ppl_now)}/{n_max}'

        suptitle = f'{studio_name}: {int(ppl_now)}/{n_max}'

    else:

        title = f'Jetzt: N/A/{n_max}'

        suptitle = f'{studio_name}: N/A/{n_max}'
